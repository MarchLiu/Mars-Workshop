...
;;      :sqli-login (user password server port database)
...
;; Improve: March Liu<march.liu@gmail.com>
;; Date: 2010-10-19
;; Content: I add the server port as a choice. And sql-postgres 
;; had add port while login.
...
(defvar sql-product-alist
  '((ansi
    ...
    (postgres
     ...
     :sqli-login (user database server port)
...
following:
 ...
 :sqli-connect          the name of a function which accepts no
                        ...
                        `sql-user', `sql-password', `sql-port',
...
The variables `sql-user', `sql-password', `sql-server', `sql-port' and
...
`sql-server-history' `sql-port-history' and `database-history'.  
...
the variables `sql-server', `sql-port', `sql-user', `sql-password', and
